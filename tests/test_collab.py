...
def workspace(db):
    ...
    Membership.objects.create(project=project, user=reviewer, role=Role.REVIEWER)
    Membership.objects.create(project=project, user=stakeholder, role=Role.STAKEHOLDER)
    data, _ = load_projection(SAMPLE)
    revision, _ = import_projection(project, data)
    return project, revision, reviewer, stakeholder


def _url(name, project, revision, extra):
    return reverse(name, args=[project.slug, revision.scenario.slug, revision.pk, *extra])


def test_reviewer_can_comment(client, workspace):
    project, revision, reviewer, _ = workspace
    client.force_login(reviewer)
    url = _url("object-comment", project, revision, ["technique", TECH])
...
def test_stakeholder_cannot_comment(client, workspace):
    project, revision, _, stakeholder = workspace
...
def test_stakeholder_cannot_set_review_state(client, workspace):
    project, revision, _, stakeholder = workspace
    client.force_login(stakeholder)
    url = _url("object-review-state", project, revision, ["technique", TECH])
...
def test_stakeholder_cannot_record_decision(client, workspace):
    project, revision, _, stakeholder = workspace
    client.force_login(stakeholder)
    url = _url("object-decision", project, revision, ["technique", TECH])
...
def test_detail_shows_comment_form_only_for_contributors(client, workspace):
    project, revision, reviewer, stakeholder = workspace
    detail = reverse(
        "technique-detail", args=[project.slug, revision.scenario.slug, revision.pk, TECH]
    )
...
def test_project_activity_view(client, workspace):
    project, revision, reviewer, _ = workspace
    client.force_login(reviewer)
    client.post(_url("object-comment", project, revision, ["technique", TECH]), {"body": "note"})
    response = client.get(reverse("project-activity", args=[project.slug]))
    assert response.status_code == 200
    assert b"commented" in response.content


def test_project_activity_requires_membership(client, workspace):
    project, _, _, _ = workspace
    outsider = User.objects.create_user(email="out@example.com", password="review-pass-1")
    client.force_login(outsider)
    assert client.get(reverse("project-activity", args=[project.slug])).status_code == 404
