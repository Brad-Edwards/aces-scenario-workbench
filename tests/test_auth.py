...
def test_dashboard_lists_only_member_projects(client, project, member):
    Project.objects.create(slug="other", name="Other Project")
    client.force_login(member)
    response = client.get(reverse("dashboard"))
    assert response.status_code == 200
    assert b"Demo Project" in response.content
    assert b"Other Project" not in response.content
...
def test_invite_accept_new_user(client, project):
    invitation = Invitation.objects.create(
        email="new@example.com", project=project, role=Role.AUTHOR
    )
...
def test_invite_accept_existing_user_grants_membership(client, project):
    existing = User.objects.create_user(email="existing@example.com", password=STRONG_PASSWORD)
    invitation = Invitation.objects.create(
        email="existing@example.com", project=project, role=Role.STAKEHOLDER
    )
    response = client.post(reverse("invite-accept", args=[invitation.token]))
    assert response.status_code == 302
    assert response.url == reverse("login")
    assert Membership.objects.filter(project=project, user=existing).exists()


def test_invite_accept_password_mismatch_creates_no_user(client, project):
    invitation = Invitation.objects.create(email="bad@example.com", project=project)
    response = client.post(
...
def test_invite_accept_weak_password_rejected(client, project):
    invitation = Invitation.objects.create(email="weak@example.com", project=project)
    response = client.post(
        reverse("invite-accept", args=[invitation.token]),
...
def test_invite_accept_rejects_used_or_expired(client, project):
    used = Invitation.objects.create(
        email="used@example.com", project=project, accepted_at=timezone.now()
    )
    assert client.get(reverse("invite-accept", args=[used.token])).status_code == 410

    expired = Invitation.objects.create(email="old@example.com", project=project)
...
def test_invitation_helpers(project):
    invitation = Invitation.objects.create(email="i@example.com", project=project)
    assert invitation.is_pending() is True
    assert invitation.is_expired() is False
    assert str(invitation) == f"Invitation for i@example.com to {project.pk}"


@pytest.mark.django_db
def test_authz_helpers(project, member):
    assert authz.is_member(member, project) is True
    assert authz.user_role(member, project) == Role.REVIEWER
    assert authz.user_membership(AnonymousUser(), project) is None
    assert authz.user_role(AnonymousUser(), project) is None
